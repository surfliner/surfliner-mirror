...
module Parse
  ...
  # We have two indicators of where the associated binary data will
  # be: the 'files' attribute from the metadata itself, and the set of
  ...
  def self.get_binary_paths(paths, data_args)
    ...
    paths.map do |path|
      # when Pathname#join is called with a path that begins with a
      # backslash, it's assumed to be an absolute path and the prefix
      # part of the path is ignored
      normalized_path = path.sub(%r{^\/}, '')

      explicit_path = Pathname.new(
      ...
      if explicit_path.exist?
        ...
        ::Parse.find_paths(data_args).select do |d|
          path.include? File.basename(d)
        end
      end
    end.flatten
  end
